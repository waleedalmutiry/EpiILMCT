...
!# DESCRIPTION:
!#
!#     To calculate the log-likelihood for the SIR continuous-time ILMs:
!#
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine loglikcontilm_f(n, ninfected, num, nsuspar, ntranspar, cc, d333, epidat,  &
    & suscov, transcov, suspar, powersus, transpar, powertrans, kernelpar, spark, deltain1, &
    & deltain2, likk) bind(C,  name="loglikcontilm_f_")

    external infinity_value

    integer (C_INT) :: j, i, r, m
    integer (C_INT), intent(in) :: n, ninfected, num, nsuspar, ntranspar    !integers
    real (C_DOUBLE), intent(in) :: deltain1, deltain2   ! parameters of the infectious period distribution
    real (C_DOUBLE), intent(in) :: spark                               ! spark & maximum infection time
    real (C_DOUBLE), intent(in), dimension(2) :: kernelpar              ! parameter of the kernel function
    real (C_DOUBLE), intent(in), dimension(n, n) :: d333, cc               ! distance & network matrices
    real (C_DOUBLE), intent(in), dimension(n, 4) :: epidat                ! epidemic data
    ...
    real (C_DOUBLE), dimension(ninfected)  :: rt, df, rrate, gamma
...
! TOTAL LOG-LIKELIHOOD:
        do i = 1,  ninfected
            gamma(i) = gammapdf ( deltain1,  deltain2,  epidat(i, 3) )
        end do

        likk = likk1 + likk2 + sum(gamma)
            ...
            do j = 1, n
                if (j .le. ninfected) then
                    if (j .ne. i) then
                        ...
                        qa2 = dot_product(transpar, transcov1)
                        rate = qa1 * qa2 * &
                            &(kernelpar(1)/((d333(int(epidat(i, 1)), int(epidat(j, 1)))**(2.0_c_double))+ &
            ...
            do j = 1, n
                if (j .le. ninfected) then
                    if (j .ne. i) then
                        ...
                        qa2 = dot_product(transpar, transcov1)
                        rate = qa1 * qa2 * &
                            &( (d333(int(epidat(i, 1)), int(epidat(j, 1)))**(-kernelpar(1))) + &
...
! TOTAL LOG-LIKELIHOOD:
    ...
    end subroutine loglikcontilm_f
...
! Gamma density function
    ...
    double precision :: alph, bet, valuelog, rval, value
      if ( rval <= 0.0d0 ) then
        ...
        value = (alph * dlog ( bet )) + (( alph - 1.0d0 ) * dlog ( rval ) ) - (bet * rval) - (valuelog)
      end if
